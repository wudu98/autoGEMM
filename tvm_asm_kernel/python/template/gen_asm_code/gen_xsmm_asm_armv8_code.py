...
def compile_time_for_init_func_asm(LINES, COLS, K, UNROLL_K, real_lines, real_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B, with_bias):
    LOOP_K_BEGIN_FLAG = 0
    LOOP_K_END_FLAG = 0
...
def compile_time_for_loop_k_begin_func_asm(LINES, COLS, K, UNROLL_K, real_lines, real_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B, with_bias):
    LOOP_K_BEGIN_FLAG = 1
    LOOP_K_END_FLAG = 0
    ...
    MAIN_K_LOOP_BEGIN = 0
    MAIN_K_LOOP_END, _ = get_loop_ID(K, UNROLL_K)
...
def compile_time_for_loop_k_main_body_func_asm(LINES, COLS, K, UNROLL_K, real_lines, real_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B):
    LOOP_K_BEGIN_FLAG = 0
    LOOP_K_END_FLAG = 0
    REG_BLOCK_TRANS_FLAG = 0
    FMA_CALCULATE_FLAG = 1
    ...
    MAIN_K_LOOP_BEGIN, _ = get_loop_ID(K, UNROLL_K)
...
def compile_time_for_loop_k_remain_func_asm(LINES, COLS, K, UNROLL_K, real_lines, real_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B):
    LOOP_K_BEGIN_FLAG = 0
    LOOP_K_END_FLAG = 0
    REG_BLOCK_TRANS_FLAG = 0
    FMA_CALCULATE_FLAG = 1
    ...
    REMAIN_K_LOOP_BEGIN, REMAIN_K_LOOP_END = get_loop_ID(K, UNROLL_K)
...
def compile_time_for_m_dim_micro_kernel_pipeline_func_asm(LINES, COLS, K, UNROLL_K, real_lines, real_cols, next_lines, next_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B, with_bias):
    LOOP_K_BEGIN_FLAG = 0
    LOOP_K_END_FLAG = 0
    REG_BLOCK_TRANS_FLAG = 1
    FMA_CALCULATE_FLAG = 1
    ...
    _, REMAIN_K_LOOP_BEGIN = get_loop_ID(K, UNROLL_K)
...
def compile_time_for_n_dim_micro_kernel_pipeline_func_asm(LINES, COLS, K, UNROLL_K, real_lines, real_cols, next_lines, next_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B, with_bias):
    LOOP_K_BEGIN_FLAG = 0
    LOOP_K_END_FLAG = 0
...
def compile_time_for_loop_k_end_func_asm(LINES, COLS, K, UNROLL_K, real_lines, real_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B):
    LOOP_K_BEGIN_FLAG = 0
    LOOP_K_END_FLAG = 1
    REG_BLOCK_TRANS_FLAG = 0
    FMA_CALCULATE_FLAG = 1
...
def m_dim_func_asm(MR_MAIN, MR_MAIN_LOOPS, MR_REMAIN, MR_REMAIN_LOOPS, NR, K, UNROLL_K, real_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B, with_bias):
    ...
    if MR_MAIN_LOOPS : # Enter the M-dim main operation
      if MR_MAIN_LOOPS > 1 : # Cyclic M-dim main operation
        ...
        code_str += compile_time_for_loop_k_remain_func_asm(MR_MAIN, NR, K, UNROLL_K, MR_MAIN, real_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B)
        code_str += f"    \"beq     3f              \\n\"\n"
        code_str += compile_time_for_m_dim_micro_kernel_pipeline_func_asm(MR_MAIN, NR, K, UNROLL_K, MR_MAIN, real_cols, MR_MAIN, real_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B, with_bias)
        code_str += f"  \"1:                                 \\n\"\n"
      ...
      if Main_K_loop_flag : 
        ...
        code_str += compile_time_for_loop_k_begin_func_asm(MR_MAIN, NR, K, UNROLL_K, MR_MAIN, real_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B, with_bias)
        code_str += f"    \"b       4f                                 \\n\"\n"
        code_str += f"  \"5:                                 \\n\"\n"
        code_str += compile_time_for_loop_k_main_body_func_asm(MR_MAIN, NR, K, UNROLL_K, MR_MAIN, real_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B)
        code_str += f"  \"4:                                 \\n\"\n"
        if MR_MAIN_LOOPS > 1 :
          code_str += f"    \"beq     2b                       \\n\"\n"
        else:
    ...
    if MR_REMAIN_LOOPS : # Enter the M-dim remain operation
      if MR_MAIN_LOOPS : # Cyclic M-dim remain operation
        code_str += compile_time_for_m_dim_micro_kernel_pipeline_func_asm(MR_MAIN, NR, K, UNROLL_K, MR_MAIN, real_cols, MR_REMAIN, real_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B, with_bias)
      if MR_REMAIN_LOOPS > 1 :
        code_str += f"    \"mov     x14, #{MR_REMAIN_LOOPS}                   \\n\"\n"
        code_str += f"    \"b       1f                                 \\n\"\n"
        code_str += f"  \"2:                                 \\n\"\n"
        code_str += f"    \"subs    x14, x14, #1                            \\n\"\n"
        code_str += compile_time_for_loop_k_remain_func_asm(MR_MAIN, NR, K, UNROLL_K, MR_REMAIN, real_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B)
        code_str += f"    \"beq     3f              \\n\"\n"
        code_str += compile_time_for_m_dim_micro_kernel_pipeline_func_asm(MR_MAIN, NR, K, UNROLL_K, MR_REMAIN, real_cols, MR_REMAIN, real_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B, with_bias)
        code_str += f"  \"1:                                 \\n\"\n"
      ...
      if Main_K_loop_flag : 
        code_str += f"    \"mov     x15, #{Main_K_loop_times}                   \\n\"\n"
        code_str += f"    \"subs    x15, x15, #1                            \\n\"\n"
        code_str += compile_time_for_loop_k_begin_func_asm(MR_MAIN, NR, K, UNROLL_K, MR_REMAIN, real_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B, with_bias)
        code_str += f"    \"b       4f                                 \\n\"\n"
        code_str += f"  \"5:                                 \\n\"\n"
        code_str += compile_time_for_loop_k_main_body_func_asm(MR_MAIN, NR, K, UNROLL_K, MR_REMAIN, real_cols, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B)
        code_str += f"  \"4:                                 \\n\"\n"
        if MR_REMAIN_LOOPS > 1 :
          code_str += f"    \"beq     2b                       \\n\"\n"
        else:
...
def n_dim_func_asm(REMAIN_N, K, UNROLL_K, NR, NR_LOOPS, MR_MAIN, MR_MAIN_LOOPS, MR_REMAIN, MR_REMAIN_LOOPS, with_bias) :
    ...
    code_str = f""
    code_str += compile_time_for_init_func_asm(MR_MAIN, NR, K, UNROLL_K, lines_branch_1, cols_branch_1, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B, with_bias)
    ...
    if Main_N_flag : # Enter the N-dim main operation
      if NR_LOOPS > 1 : # Cyclic N-dim main operation
        ...
        code_str += f"    \"beq     7f                       \\n\"\n" 
        code_str += compile_time_for_n_dim_micro_kernel_pipeline_func_asm(MR_MAIN, NR, K, UNROLL_K, lines_branch_2, SIMD_LANE*NR, lines_branch_1, SIMD_LANE*NR, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B, with_bias)
        code_str += f"  \"6:                                 \\n\"\n"
    ...
    if Edge_N_flag : # Enter the N-dim remain operation
      if Main_N_flag : # Cyclic N-dim remain operation
        code_str += compile_time_for_n_dim_micro_kernel_pipeline_func_asm(MR_MAIN, NR, K, UNROLL_K, lines_branch_2, SIMD_LANE*NR, lines_branch_1, Edge_N, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B, with_bias)
    ...
    code_str += compile_time_for_loop_k_end_func_asm(MR_MAIN, NR, K, UNROLL_K, lines_branch_2, cols_branch_2, vector_id_array_A, VEC_REG_A_LEN, vector_scroll_B, register_scroll_B)
...
def xsmm_asm_armv8_code(M, K, N, with_bias, with_relu, lda, ldb, ldc, uniq_id):
    UNROLL_K = 8
    NR_MAIN = 4
