...
# compile by g++
# corresponding to optimization listing 3
# corresponding to figure b) 
...
def compile_time_for_unroll_k(M, N, K, ROWS, COLS):
    ...
    for row in range (ROWS) :
      code_str += f"      va[{row}] = vld1q_f32(A + {row} * lda);\n"
    for i in range(4):
      for col in range (COLS) :
        code_str += f"      vb[{col}] = vld1q_f32(B + {col} * 4);\n"
    code_str += f"    for (int k = 0; k < {K//4}; k++) {{\n"     
    for i in range(4):
      for col in range (COLS) :
        for row in range (ROWS) :
          code_str += f"      vc[{row*COLS+col}] = vmlaq_laneq_f32(vc[{row*COLS+col}], vb[{col}], va[{row}], {i});\n"
        code_str += f"      vb[{col}] = vld1q_f32(B + (k * 4 + {i} + 1) * ldb + {col} * 4);\n"
    for row in range (ROWS) :
      code_str += f"      va[{row}] = vld1q_f32(A + {row} * lda + (k+1) * 4);\n"
    code_str += f"    }}\n"
